...
export const ActiveUsersPanel: React.FC<ActiveUsersPanelProps> = ({
  ...
  return (
    <div id="active-gladiators-panel" className="bg-[#111827] border border-gray-800 rounded-2xl p-6 shadow-xl space-y-6">
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            ...
            <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest font-bold block mt-0.5">
              Refreshed Lives • {users.filter(u => u.status === 'online' && u.uid !== currentUserId).length} Active Now
            </span>
          </div>
        </div>
      ...
      {activeDirectory.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 gap-4 max-h-[420px] overflow-y-auto pr-1">
          {activeDirectory.map((u) => {
            const isChallenging = activeChallengeUserId === u.uid;
            
            return (
              <div 
                key={u.uid}
                id={`gladiator-card-${u.uid}`}
                className={`p-4 rounded-2xl border transition duration-150 space-y-3 flex flex-col justify-between ${
                  isChallenging 
                    ? 'bg-slate-950 border-amber-500/50 shadow-lg shadow-amber-950/10' 
                    : 'bg-slate-950/45 border-gray-800 hover:border-gray-700/80'
                }`}
              >
                ...
                {/* Challenge Parameters Section Overlay inside card */}
                {isChallenging && (
                  <div className="bg-slate-950 border-t border-gray-900 pt-3 space-y-4 animate-in slide-in-from-top duration-200">
                    <span className="text-[10px] font-mono text-amber-500 uppercase tracking-widest block font-bold">
                      Challenge Board Settings
                    </span>

                    {/* Challenge Bet presets */}
                    <div className="space-y-1.5">
                      <label className="text-[9px] font-mono text-gray-400 uppercase tracking-wider block">Choose Bet Size (₦)</label>
                      <div className="grid grid-cols-4 gap-1">
                        {[10, 50, 200, 500].map(amt => (
                          <button
                            ...
                            className={`py-1 rounded-md border text-[10px] font-bold font-mono transition cursor-pointer ${
                              challengeBet === amt 
                      ...
                      <div className="relative mt-2">
                        <span className="absolute left-2.5 top-1.5 text-xs text-gray-500 font-mono font-bold">₦</span>
                        <input
                          type="number"
                          min="0"
                          value={challengeBet}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1">
                        <label className="text-[9px] font-mono text-gray-400 uppercase block">Game Clock</label>
                        <select
                          value={challengeTime}
                          onChange={(e) => setChallengeTime(e.target.value)}
                          className="w-full bg-slate-950 border border-gray-900 rounded-lg py-1.5 px-2 text-[11px] text-white focus:outline-none focus:border-amber-500 font-mono"
                        >
                      ...
                      {/* Side Preference */}
                      <div className="space-y-1">
                        <label className="text-[9px] font-mono text-gray-400 uppercase block">Play Color</label>
                        <select
                    ...
                    {challengeError && (
                      <div className="p-2 bg-red-950/30 border border-red-900/30 text-red-500 text-[10px] rounded-lg">
                        {challengeError}
                      </div>
                    )}

                    {successMsg && (
                      <div className="p-2 bg-emerald-950/20 border border-emerald-900/40 text-emerald-400 text-[10px] rounded-lg flex items-center gap-1.5">
                        <Check className="w-3.5 h-3.5 flex-shrink-0" />
                        <span>{successMsg}</span>
                      </div>
                    )}

                    <div className="flex gap-2 pt-1 border-t border-gray-950/80">
                      <button
                        type="button"
                        onClick={() => handleOpenChallenge(u.uid)}
                        className="flex-1 py-1.5 rounded-xl border border-gray-800 text-[11px] text-gray-400 hover:text-white transition cursor-pointer font-bold"
                      >
                        Cancel
                      </button>
                      <button
                        type="button"
                        onClick={() => handleTriggerChallenge(u)}
                        disabled={isSubmitting || !!successMsg}
                        className="flex-1 py-1.5 bg-amber-500 hover:bg-amber-400 disabled:bg-gray-800 disabled:text-gray-500 transition text-slate-950 font-display font-extrabold text-[11px] rounded-xl cursor-pointer flex items-center justify-center gap-1"
                      >
                        {isSubmitting ? (
